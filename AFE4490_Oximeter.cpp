...
boolean AFE4490 :: get_AFE4490_Data (afe44xx_data *afe44xx_raw_data,const int chip_select,const int data_ready)
{
  ...
  // Hence 100 samples is 4 sec worth  
  if (dec == DECIMATE)
  {
    aun_ir_buffer[dec_buffer_count] = (uint16_t) ((afe44xx_raw_data->IR_data) >> 4);  // ? should this be >>6 22 bits to 16 
    aun_red_buffer[dec_buffer_count] = (uint16_t) ((afe44xx_raw_data->RED_data) >> 4);
    dec_buffer_count++;
    dec = 0;
  }

  dec++;
   // When Buffer has 10 seconds of data 
  if (dec_buffer_count > IR_BUFFER_FULL)
  {
    // Call Routine to estimate spO2 and heart rate 
    // Pass to routine:
    // IR buffer, Buffer length = 100, Red buffer, Pointers to pO2 sats, pO2 sats flag, Heart rate, heart rate flag  
    Spo2.estimate_spo2(aun_ir_buffer, IR_BUFFER_FULL, aun_red_buffer, &n_spo2, &ch_spo2_valid, &n_heart_rate, &ch_hr_valid);
