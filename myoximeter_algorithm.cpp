...
/*void spo2_algorithm :: estimate_spo2(uint16_t *pun_ir_buffer, int n_ir_buffer_length, uint16_t *pun_red_buffer, int8_t *pn_spo2, bool *pch_spo2_valid, int8_t *pn_heart_rate, bool *pch_hr_valid)
{
...
*/
    int pn_spo2 = internal_data->n_spo2;
    int pn_heart_rate = internal_data->n_heart_rate;
    
    int n_ir_buffer_length = internal_data->buffer_length;
    bool pch_spo2_valid = internal_data->ch_spo2_valid;
    bool pch_hr_valid = internal_data->ch_hr_valid;
   
  // calculates mean of all samples and subtract from IR Buffer - effectively approx DC removal
  ...
  // as buffer length is 127 - divide by right shifting 7 bits 
  DC_ir_mean = (DC_ir_mean>>7);
